...
export const AudioPlayer: React.FC = () => {
  ...
  // Floating position state for compact audio button
  ...
  const handleStart = (clientX: number, clientY: number) => {
    ...
    const currentX = position ? position.x : 16;
    const currentY = position ? position.y : Math.max(16, window.innerHeight - 150);
    ...
    return (
      <>
        <div
          ...
          className={
            position
              ? 'flex flex-col items-start gap-1 select-none animate-fade-in'
              : 'fixed bottom-24 sm:bottom-28 left-4 sm:left-6 z-40 flex flex-col items-start gap-1 select-none animate-fade-in'
          }
        >
          {/* Drag handle header */}
          <div
            ...
            className="cursor-grab active:cursor-grabbing px-2.5 py-1 rounded-full bg-slate-950/90 text-amber-300 text-[10px] font-bold flex items-center gap-1 shadow-lg border border-amber-500/40 backdrop-blur-md hover:bg-slate-900 transition-colors"
            title="Click or touch and drag to move audio player anywhere on screen"
          >
            <Move className="w-3 h-3 text-amber-400 animate-pulse" />
            <span>Drag Audio Button</span>
          </div>

          {/* Compact Floating Audio Pill */}
          <div className="flex items-center gap-2.5 p-2 pr-3.5 rounded-full bg-[#0A2342] text-white border-2 border-amber-500/60 shadow-2xl backdrop-blur-md">
            <div 
              className={`w-9 h-9 rounded-full bg-amber-500 text-slate-950 flex items-center justify-center font-bold relative shrink-0 overflow-hidden ${isPlaying ? 'animate-spin-slow' : ''}`}
            >
            ...
            >
              <span className="text-[10px] text-amber-300 font-semibold uppercase tracking-wider flex items-center gap-1">
                ...
                <span>{isPlaying ? 'Playing Audio' : 'Audio Paused'}</span>
              </span>
            ...
            <button
              onClick={togglePlay}
              className="w-8 h-8 rounded-full bg-amber-400 text-slate-950 flex items-center justify-center font-bold hover:scale-105 active:scale-95 transition-transform shrink-0"
            ...
            <button
              ...
              title="Expand full audio player bar"
            >
              ...
              <div className="space-y-5 text-sm text-slate-700 dark:text-slate-300">
                <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 space-y-2">
                  <div className="flex items-center gap-2 font-bold text-amber-900 dark:text-amber-300">
                    <span>Google Drive, Dropbox & Uploaded MP3 Support</span>
                  </div>
                  <p className="text-xs">
                    All audio formats, drive links, and uploaded files stream smoothly.
                  </p>
                </div>
              </div>
  ...
  return (
    <>
      ...
      >
        ...
        {/* Minimize / Make Floating Bar Tab */}
        <button
          onClick={() => setIsMinimized(true)}
          className="absolute -top-6 right-8 px-4 py-1 rounded-t-lg bg-[#0A2342] border-t border-x border-amber-500/40 text-[11px] font-bold uppercase tracking-wider text-amber-300 flex items-center gap-1.5 hover:text-white transition-colors"
          style={{ backgroundColor: '#0A2342' }}
          aria-label="Make Audio Player Floating & Movable"
        >
          <Move className="w-3 h-3 text-amber-400 animate-pulse" />
          <span>Make Floating & Movable</span>
